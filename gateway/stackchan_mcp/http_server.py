...
This module is deliberately a lean ASGI concern: how the MCP server, the
single-flight device command queue, the health/status endpoints and the
host/Origin bearer-token guard fit together. The dashboard ``/control/*``
REST contract lives in :mod:`stackchan_mcp.control_http`, and the shared
JSON/field helpers it (and other HTTP paths) reuse live in
:mod:`stackchan_mcp.http_common`.
"""
...
from .control_http import build_control_routes
...
def build_app(
    ...
    *,
    ...
    routes = [
        Route(
            "/mcp",
            _StreamableHTTPASGIApp(session_manager),
            methods=["GET", "POST", "DELETE"],
        ),
        Route("/healthz", healthz, methods=["GET"]),
        Route("/status", status, methods=["GET"]),
        # Phase F dashboard control routes (token-guarded by prefix).
        *build_control_routes(gateway, dispatch_fn),
    ]
