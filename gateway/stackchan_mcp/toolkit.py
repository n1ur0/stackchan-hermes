...
TOOLS: list[ToolDef] = [
    ...
    ToolDef('set_subtitle', "Show a multi-line caption on the device screen's bottom subtitle box (what the persona is currently speaking). Long text wraps and auto-scrolls to follow the speech. Pass an empty string to clear it. Used by the gateway's voice pipeline (Phase F); safe to call directly too.", '{"type": "object", "properties": {"text": {"type": "string", "description": "Subtitle caption to show; empty string clears it"}}, "required": ["text"]}', relay='self.display.set_subtitle'),
